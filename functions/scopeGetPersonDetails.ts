...
Deno.serve(async (req) => {
  try {
    ...
    console.log('🔄 Getting person details from Scope:', personId);
    ...
    while (attempts < maxPolls && !screeningComplete) {
      attempts++;
      console.log(`📊 Polling attempt ${attempts}/${maxPolls}`);
      ...
      if (!detailsResponse.ok) {
        ...
        console.error('❌ Scope details failed:', detailsResponse.status, errorText);
      ...
      console.log('📊 Screening status:', screeningStatus);
    ...
    if (!screeningComplete) {
      console.warn('⏰ Screening timeout - max polls reached');
      return Response.json({ 
